...
Matrix* matrix_multiply(Matrix *A, Matrix *B) {
    ...
    // Triple bucle anidado (Lo que consume tiempo)
...
void mlp_forward(MLP *net, Matrix *X) {
    ...
    memcpy(net->A1->data, net->Z1->data, net->Z1->rows * net->Z1->cols * sizeof(double));
    apply_relu(net->A1);
    ...
    memcpy(net->A2->data, net->Z2->data, net->Z2->rows * net->Z2->cols * sizeof(double));
...
void mlp_backward(MLP *net, Matrix *X, Matrix *Y, double lr) {
    ...
    memcpy(dZ2->data, net->A2->data, net->A2->rows * net->A2->cols * sizeof(double));
    // Restamos Y manualmente
    for(int i=0; i<dZ2->rows * dZ2->cols; i++) dZ2->data[i] -= Y->data[i];
...
int main() {
    ...
    if (!f_img || !f_lbl) {
        ...
        printf("Asegurate de que NO tengan extension .gz (descomprimelos)\n");
        return 1;
    }
    ...
    read_int(f_lbl); // Num items

    // Limitar datos para prueba rápida (C es lento sin optimizaciones)
    int LIMIT = 60000; 
    ...
    printf("\n--- RESULTADO FINAL C PURO ---\n");
    printf("Tiempo: %.4f segundos\n", time_taken);

    // Limpieza final (Buen ciudadano de C)
    free(y_temp);
    matrix_free(X_train);
    matrix_free(Y_train);
    // mlp_free(mlp); // Tarea: Implementar función que libere todo dentro de MLP
